...
class DiplomaticRepresentation:
    def __init__(self, csv_row):
        cells = csv_row.rstrip().split(
            ";"
        )  # rstrip removes the trainling \n character.
    ...
    def url_name(self, include_city, include_jurisdiction):
        ...
        if include_city:
            name += (
                safe_for_url(self.operator)
                + "_in_"
                + safe_for_url(self.city)
                + "_"
                + safe_for_url(self.country)
            )
        else:
...
for operator in world.operators.values():
    ...
    for country in operator.countries.values():
        ...
        for diplo in country.values():
            ...
            include_jurisdiction = (
                len(diplo.jurisdictions) > 1
            )  # and diplo.jurisdictionQIDs != diplo.countryQID
            ...
            if os.path.exists(path):
                ...
                with open(path, "r") as file:
                    ...
                    QID2 = re.search(
                        r'http://www.wikidata.org/entity/Q[^"]*', html
                    ).group(0)
                    print(
                        "Error: Several diplomatic representations for path "
                        + path
                        + ": "
                        + QID1
                        + " "
            ...
            if diplo.website:
                file.write(
                    '<p><a href="' + diplo.website + '">Official website</a></p>\n'
                )
            if diplo.facebook:
                file.write(
                    '<p><a href="https://www.facebook.com/'
                    + diplo.facebook
                    + '">Official Facebook account</a></p>\n'
                )
            if diplo.twitter:
                file.write(
            ...
            if diplo.youtube:
                file.write(
                    '<p><a href="https://www.youtube.com/channel/'
                    + diplo.youtube
                    + '">Official YouTube account</a></p>\n'
                )
            if diplo.picture:
                file.write(
                    '<p><a href="'
            ...
            if diplo.pictureLicense:
                ...
                if diplo.pictureLicenseURL:
                    license = (
                        '<a href="' + diplo.pictureLicenseURL + '">' + license + "</a>"
                    )
            ...
            file.write(
                '<p><i>Any error or missing information? Please improve <a href="'
                + diplo.QID
                + '">this Wikidata item</a>, or let us know on the <a href="https://github.com/database-of-embassies/database-of-embassies/issues">issue tracker</a>.<br/>\n'
            )
            file.write(
                'Public domain. Feel free to reuse this data in any way you want. Download the <a href="https://raw.githubusercontent.com/database-of-embassies/database-of-embassies/master/database_of_embassies.csv">whole CSV data</a> or browse <a href="index.html">other diplomatic representations</a>.</i></p>\n'
            )
            file.write(
                '<p><a href="'
                + diplo.QID
                + '"><img src="https://upload.wikimedia.org/wikipedia/commons/thumb/4/41/Wikidata_Stamp_Rec_Light.svg/200px-Wikidata_Stamp_Rec_Light.svg.png"/></a></p>\n'
            )
